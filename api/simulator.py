...
from api.api import Api, implementer
import numpy as np
...
class Simulator:
    ...
    def reset(self, sfc_n: int = 4) -> None:
        ...
        while cpu_load < MAX_EDGE_LOAD and mem_load < MAX_EDGE_LOAD:
            # VNF를 생성
            vnf_type = np.random.choice(POSSIBLE_VNF_TYPE)
    ...
    def move_vnf(self, vnf_id: int, srv_id: int) -> bool:
        # vnf_id가 존재하는지 확인
        if vnf_id >= len(self.srvs):
            return False
        # srv_id가 존재하는지 확인
        if srv_id >= len(self.srvs):
            return False
        # 해당 srv에 이미 vnf가 존재하는지 확인
        for vnf in self.srvs[srv_id]:
            if vnf.id == vnf_id:
                return False
        # capacity 확인
        srv_remain_cpu_cap, srv_remain_mem_cap = self._calc_remain_cap_from_srv()
        if srv_remain_cpu_cap < self.srvs[vnf_id].cpu_req or srv_remain_mem_cap < self.srvs[vnf_id].mem_req:
            return False
    ...
    def _calc_edge_load(self) -> List[float, float]:
        edge_cpu_cap = 0
        edge_mem_cap = 0
        edge_cpu_req = 0
        edge_mem_req = 0
    ...
    def _calc_remain_cap_from_srv(self, srv_id) -> List[float, float]:
