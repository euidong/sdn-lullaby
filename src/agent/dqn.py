...
class DQNAgent:
    ...
    BATCH_SIZE = 32
    vnf_selection_input_num = 11
    vnf_placement_input_num = 11
    ...
    def __init__(self, srv_num: int, init_epsilon: float = 0.5, final_epsilon: float = 0.1, vnf_s_lr: float = 0.01, vnf_p_lr: float = 0.01, alpha=0.1, gamma=1.0) -> None:
        ...
        self.vnf_selection_model = NN(
            self.vnf_selection_input_num, 1, num_layers=4).to(self.device)
        self.vnf_placement_model = NN(
            self.vnf_placement_input_num, 1, num_layers=4).to(self.device)
        self.vnf_selection_optimizer = torch.optim.Adam(
            self.vnf_selection_model.parameters(), lr=vnf_s_lr)
        self.vnf_placement_optimzer = torch.optim.Adam(
            self.vnf_placement_model.parameters(), lr=vnf_p_lr)
    ...
    def decide_action(self, state: State, epsilon_sub: float) -> Action:
        ...
        if is_random:
            ...
            with torch.no_grad():
                vnf_s_out = self.vnf_selection_model(
                    vnf_s_in.unsqueeze(0))[0]
        ...
        if is_random:
            ...
            with torch.no_grad():
                vnf_p_out = self.vnf_placement_model(
                    vnf_p_in.unsqueeze(0))[0]
    ...
    def update(self, _state, _action, reward, _next_state) -> None:
        ...
        vnf_selection_q = self.vnf_selection_model(
            vnf_s_in_batch)[0].gather(1, vnf_s_out_batch)
        vnf_placement_q = self.vnf_placement_model(
            vnf_p_in_batch)[0].gather(1, vnf_p_out_batch)

        # calculate next_state-max_action value
        vnf_selection_expect_q = reward_batch + self.gamma * \
            self.vnf_selection_model(next_vnf_s_in_batch)[0].max(1)[
                0].detach().unsqueeze(1)
        vnf_placement_expect_q = reward_batch + self.gamma * \
            self.vnf_placement_model(next_vnf_p_in_batch)[0].max(1)[
                0].detach().unsqueeze(1)
    ...
    def save(self) -> None:
        torch.save(self.vnf_selection_model.state_dict(),
                   "param/vnf_selection_model.pth")
        torch.save(self.vnf_placement_model.state_dict(),
                   "param/vnf_placement_model.pth")

    def load(self) -> None:
        self.vnf_selection_model.load_state_dict(
            torch.load("param/vnf_selection_model.pth"))
        self.vnf_selection_model.eval()
        self.vnf_placement_model.load_state_dict(
            torch.load("param/vnf_placement_model.pth"))
        self.vnf_placement_model.eval()
    ...
    def _convert_state_to_vnf_placement_input(self, state: State, vnf_id: int) -> torch.Tensor:
        vnf_placement_input = torch.zeros(
            self.srv_num, self.vnf_placement_input_num)
